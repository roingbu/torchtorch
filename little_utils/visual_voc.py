...
def save_viz_image(image_path,xml_path,save_path):
    ...
    for info in bbox:
        ...
        #cv2.putText(image,info[0],(info[1],info[2]),cv2.FONT_HERSHEY_PLAIN,(0,0,255),2)
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    cv2.imwrite(os.path.join(save_path,image_path.split('/')[-1]),image)

if __name__ == '__main__':
    image_dir = '/data/slam_dataset/visdrone/VOC_VisDrone/JPEGImages'
    xml_dir = '/data/slam_dataset/visdrone/VOC_VisDrone/Annotations'
    save_dir = '/data/slam_dataset/visdrone/VOC_VisDrone/viz_images'
    image_list = os.listdir(image_dir)[:10]
    for i in  image_list:
