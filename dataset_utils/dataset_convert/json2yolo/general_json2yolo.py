...
def convert_coco_json(json_dir='../coco/annotations/', use_segments=False, cls91to80=False):
    ...
    for json_file in sorted(Path(json_dir).resolve().glob('*.json')):
        fn = Path(save_dir) / 'labels' / json_file.stem.replace('instances_', '')  # TODO:folder name
        fn.mkdir()
        os.mkdir(Path(save_dir) / 'labels' / json_file.stem.replace('instances_', '')  / PROJECT_NAME)  #TODO: added (可能要删)
        ...
        for img_id, anns in tqdm(imgToAnns.items(), desc=f'Annotations {json_file}'):
            ...
            for ann in anns:
                # print(ann)
                if ann['iscrowd']:  #TODO : RLE to polygon(iscrowd = 0)
                    ann = rle_to_coco(ann)
                    # print(ann)
                ...
                cls = coco80[ann[0]['category_id'] - 1] if cls91to80 else ann[0]['category_id'] - 1  # class
                # box = [cls] + box.tolist()
                # if box not in bboxes:
                #     bboxes.append(box)
                # Segments
                if use_segments:
                    if len(ann[0]['segmentation']) > 1:
                        s = merge_multi_segment(ann[0]['segmentation'])
                        s = (np.concatenate(s, axis=0) / np.array([w, h])).reshape(-1).tolist()
                    else:
                        s = [j for i in ann[0]['segmentation'] for j in i]  # all segments concatenated
...
if __name__ == '__main__':
    source = 'COCO'
    PROJECT_NAME = '2023-02-24-16-last-highest'

    if source == 'COCO':
        convert_coco_json('COCO/annotations',  # directory with *.json   NO RLE
                          use_segments=True,
                          cls91to80=True)
